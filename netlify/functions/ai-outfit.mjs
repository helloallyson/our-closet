  ...
  try {
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'system',
          content: `You are a fun, creative personal stylist for ${person || 'the user'}. You help put outfits together from their existing wardrobe. Be specific about why pieces work together (color coordination, style matching, occasion and weather appropriateness). Keep it conversational and encouraging.`
        }, {
          role: 'user',
          content: `Here is my wardrobe:\n${itemSummaries}${weatherContext}\n\nPut together an outfit for: ${occasion || 'a casual day out'}.\n\nReturn ONLY a JSON object (no markdown, no backticks) with:\n- "outfitName": a fun creative name for this outfit\n- "itemIds": array of item IDs (the bracketed IDs above) to include\n- "reasoning": 2-3 sentences about why these pieces work together, mentioning weather if relevant\n- "stylingTips": one sentence with a specific styling tip`
        }],
        max_tokens: 500,
        temperature: 0.7
      })
